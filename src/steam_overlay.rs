...
pub fn create_overlay_window(app: &tauri::AppHandle<Wry>) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn init() -> TauriPlugin<Wry, Option<Config>> {
    tauri::plugin::Builder::<Wry, Option<Config>>::new("steam-overlay")
        ...
        .on_event(|app_handle, event| match event {
            RunEvent::WindowEvent {
                ...
                // 1. Prevent wgpu layout crashes when window minimized (0x0 dimensions)
            ...
            // Redraw
            // Need to check if this is necessary or if just having a surface is good enough.
            RunEvent::MainEventsCleared => {
                if let Some(wgpu_state) = app_handle.try_state::<Arc<WgpuState>>() {
                    ...
                    let scale_factor = _window.current_monitor().unwrap().unwrap().scale_factor();

                    let window_position: LogicalPosition<f64> =
                        _window.inner_position().unwrap().to_logical(scale_factor);
                    let window_size: LogicalSize<f64> =
                        _window.inner_size().unwrap().to_logical(scale_factor);
